...
<table class='table'>
	...
	$getDataDetail = $this->model_invoice->getInvoiceByBulanTahun(date("m",strtotime($data->invoice_date_tt)), date("Y",strtotime($data->invoice_date_tt)));

	$no = 0;

	$sisaaa = 0;
	//echo date("Y",strtotime($data->invoice_date_tt));
	...
	foreach($getDataDetail->result() as $dataDetail){
		...
		if($tanggal_f > $tanggal_t){
			...
			$getDataDetailOri = $this->model_invoice->getInvoiceOriByBulanTahun(date("m",strtotime($data->invoice_date_tt)), date("Y",strtotime($data->invoice_date_tt)));
	...
	<tr>
		...
		<td colspan='4' align='right'><h4><b>Sisa Piutang ".date("F Y",strtotime($data->invoice_date_tt))."</b></h4></td>

		<td colspan='2' align='right'><h4><b>Rp. ".number_format($totalSemuanya,0,',','.')."</b></h4></td>
