...
<table width='100%'>
	...
	<tr>
		...
		<th align='left'><h1>".$dataRR->row()->ptname."</h1></th>

		<th align='right'><h4>".$jenis_out." ".str_replace("PT.E","PT.ETC",$dataRR->row()->nomor_retur_revisi)."</h4></th>
...
<table width='100%'>
	...
	<tr>
		...
		<th align='right'>".date("d M Y", strtotime("+0 day", strtotime($dataRR->row()->tgl)))." <br> Nama Customer : ".$dataRR->row()->nama_member." <br> ".$dataRR->row()->nama_kota." <br> Ex_Inv : ".str_replace("PT.E","PT.ETC",$dataRR->row()->no_nota)."</th>
		  ...
		  ";if($jenis == "RET"){
				...
				if($data->qty_before == $data->qty_change){
					if($data->price_before > $data->price_change){
						...
						$total_selisihh = $total_qty * $data->price_change;
						$total_selisih = "-".$total_selisihh;
			...
			if($jenis == "RET"){
				...
				$total_pembayarans = $total_pembayaran;

			}else{

				$total_pembayaran = $total_pembayaran + $total_selisih;
				if($total_pembayaran < 0){
					$total_pembayarans = "(Rp. ".number_format($total_pembayaran,0,',','.').")";
				}else{
					$total_pembayarans = "Rp. ".number_format($total_pembayaran,0,',','.')."";
				}
...
<table border='0' width='100%'>
    ...
    if($grandTotal < 0){
		$grandTotals = "(Rp. ".number_format($grandTotal,0,',','.').")";
	}else{
		$grandTotals = "Rp. ".number_format($grandTotal,0,',','.')."";
	}
	...
	<tr>
		...
		<td align='right'>
			...
			<p style='font-size: 13px;'><b>TOTAL : ".str_replace("-","",$total_pembayarans)."</b></p>
	...
	<tr>
		...
		<td align='right'>
			...
			<p style='font-size: 13px;'><b>PPN 10% : ".str_replace("-","",$ppns)."</b></p>
	...
	<tr>
		...
		<td align='right'>
			...
			<p style='font-size: 13px;'><b>GRAND TOTAL : ".str_replace("-","",$grandTotals)."</b></p>
