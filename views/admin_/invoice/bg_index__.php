...
//$SalesById = $this->model_master->getSalesById($_SESSION['rick_auto']['filter_sales'])->row();
	echo"
		...
		<div style='margin-bottom:1%;margin-top:1%' class='col-sm-12'>
			...
			<div class='col-sm-2'>
				<input type='text' class='form-control' placeholder='No. Invoice' id='invoice_no' name='invoice_no' value='".$_SESSION['rick_auto']['filter_invoice_no']."'>
			</div>
			<div class='col-sm-2' style='margin-top:1%'>
				Perusahaan 
			</div>
		...
		<div class='col-sm-12'>
			<div class='panel invoice-grid'>
				<div class='panel-body'>
						...
						<div class='col-sm-11'>
							<div class='col-sm-7'>
							<h6 class='text-semibold no-margin-top'>".$invoice->member_name." - ".$invoice->kota."</h6>
						...
						<div class='col-sm-12'>
							...
							echo"
							<h6 class='text-semibold text-right no-margin-top'>Rp. ".number_format($total_semua->total_harga,2,',','.')."</h6>";
							if($invoice->pay_status == 0){
								$st_inv = "Belum Lunas";
								$st_ic = "warning";
	...
	if($status == 0){
		// if(isset($_SESSION['rick_auto']['filter_bulan'])){
		// 	$total_inv = $this->db->query("select sum(total) as total_invoice, count('*') as jumlah_invoice from invoice where member_id = '".base64_decode($this->uri->segment(4))."' and pay_status = 0 and MONTH(dateorder) =".$_SESSION['rick_auto']['filter_bulan']."")->row();
  //       }else{
  //           $total_inv = $this->db->query("select sum(total) as total_invoice, count('*') as jumlah_invoice from invoice where member_id = '".base64_decode($this->uri->segment(4))."' and pay_status = 0 and MONTH(dateorder) =".date('m', strtotime(date('Y-m-d')))."")->row();
  //       }
		if(isset($_SESSION['rick_auto']['filter_start_date']) && $_SESSION['rick_auto']['filter_end_date']){
			$tanggal_filter = "and dateorder >= ".$_SESSION['rick_auto']['filter_start_date']." and dateorder <= ".$_SESSION['rick_auto']['filter_end_date']."";
		}else{
			$tanggal_filter = "";
		}
		if(isset($_SESSION['rick_auto']['filter_sales'])){
			...
			$array = explode(',', $_SESSION['rick_auto']['filter_sales']);
			   
			$sales_filter = "and sales_id IN ('".$string."')";
		}else{
			$sales_filter = "";
		}
