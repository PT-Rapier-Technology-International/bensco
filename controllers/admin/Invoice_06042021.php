...
class Invoice extends CI_Controller
{
    ...
    public function saveReturRevisi()
    {
        ...
        $updateInvoice = $this->db->set('sub_total', $txtTotalPembayaran)
            ->set('total_before_ppn', $txtTotalPembayaran)
            ->set('total', $grandTotal)
            ->where('id', $txtidInvoice)
            ->update('invoice');
        ...
        if ($updateInvoice) {
            ...
            foreach ($invoiceDetail->result() as $detailInvoice) {
                ...
                if ($txtTotalSatuan == $txtTotalSatuanOld && $txtQty == $txtQtyOld && $txtTotal == $txtTotalOld) {
                    ...
                    $insertInvoiceRR = $this->db->set('nomor_retur_revisi', $txtnoTransaksi)
                        ->set('invoice_tanda_terima_id', $getdataTT->id)
                        ->set('invoice_id', $txtidInvoice)
                        ->set('invoice_detail_id', $detailInvoice->id)
                        ->set('note', $txtNote)->set('qty_before', $txtQtyOld)
                        ->set('qty_change', $txtQty)
                        ->set('price_before', $txtTotalSatuanOld)
                        ->set('price_change', $txtTotalSatuan)
                        ->set('total_before', $txtTotalOld)
                        ->set('total_change', $txtTotal)
                        ->set('create_date', date("Y-m-d H:i:s"))
                        ->set('create_user', $_SESSION['rick_auto']['username'])
                        ->insert('invoice_retur_revisi');

                    $updateInvoice = $this->db->set('qty_kirim', $txtQty)
                        ->set('price', $txtTotalSatuan)
                        ->set('ttl_price', $txtTotal)
                        ->where('id', $detailInvoice->id)
                ...
                $produk_id = $this->model_produk->getProductByCode($detailInvoice->product_code)->row();
                if ($stokPeng > 0) {
                    $insert_opname_stok_bm = $this->db->set('product_id', $produk_id->id)
                        ->set('transaction_no', $txtnoTransaksi)
                        ->set('gudang_id', $detailInvoice->gudang_id)
                        ->set('perusahaan_id', $getInvoice->perusahaan_id)
                        ->set('purchase_detail_id', 0)
                        ->set('stock_input', $stokPeng)
                        ->set('note', 'Retur Barang Masuk')
                        ->set('keterangan', 'Retur Barang Masuk dari No. Invoice ' . $txtnoInvoice . '')
                        ->set('invoice_id', $txtidInvoice)->set('create_date', date("Y-m-d H:i:s"))
                        ->set('create_user', $_SESSION['rick_auto']['username'])
                        ->insert('report_stok_bm_bl');
                }
            }

            $insertPayment = $this->db->set('no_tanda_terima', $txtnoTT)
                ->set('member_id', $txtMemberId)
                ->set('payment_id', $txtJenis)
                ->set('payment_date', $txtPaymentDate)
                ->set('cicilan_ke', $hitung)
                ->set('filter_date', '0000-00-00 00:00:00')
                ->insert('invoice_payment');

            $ppn = $txtTotalPembayarans * 10 / 100;
            $grandTotal = $txtTotalPembayarans + $ppn;

            $updateInvoice = $this->db->set('sub_total', $txtTotalPembayarans)
                ->set('total_before_ppn', $txtTotalPembayarans)
                ->set('total', $grandTotal)
                ->where('id', $txtidInvoice)
                ->update('invoice');
            $insert_role = $this->db->set('no_transaction', $dataInv->nonota)
    ...
    public function saveReturRevisi_2021_03_31()
    {
        $txtnoTT = $this->input->post('txtnoTT');
        $txtnoTransaksi = $this->input->post('txtnoTransaksi');
        $txtnoInvoice = $this->input->post('txtnoInvoice');
