...
class Purchase extends CI_Controller {
    ...
    function __construct(){
        ...
        $this->load->model('admin/model_datatable_po');
   ...
   public function process_cancel_poo(){
        $id = $this->input->post('id');
        $dPurchase = $this->model_purchase->getPurchaseByID($id)->row();
        $update = $this->db->set('status_gudang',2)->set('status',2)->where('id',$id)->update('transaction_purchase');
        $txtNoteCancel = $this->input->post('txtNoteCancel');
        $getInvoice = $this->model_purchase->getPurchaseByID($id);
        $detailInvoice = $this->model_purchase->getPurchaseDetailByPurchaseID($id);
        foreach($detailInvoice->result() as $detailInv){
    ...
    public function simpanPembuatanPO(){
        ...
        foreach($getProses->result() as $gProses){
            ...
            $this->db->query("select id from transaction_purchase_temporary_detail where id = ".$id_recent."");

            if ($id_recent == 0 || $id_recent == null || $id_recent == ""){

                $this->db->query("delete from transaction_purchase_temporary where id = ".$id_recent."");
            }
    ...
    public function print_report_monthly_po(){
        ...
        $this->template->print2pdf('Print_PDF',$content, 'Report_Bulanan_PO_'.date("d M Y",strtotime("+0 day", strtotime($_SESSION['rick_auto']['bulan']))));
        }
    }
    ...
    public function getDataPO(){
        ...
        $output = array(
            ...
            "recordsTotal" => $this->model_datatable_po->count_all(),

            "recordsFiltered" => $this->model_datatable_po->count_filtered(),
