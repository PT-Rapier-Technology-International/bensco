...
class Model_datatable_po extends CI_Model {
 ...
 (
		...
		SELECT `tp`.*, `m`.`name` as `nama_member`, `s`.`name` as `nama_sales`, `x`.`name` as `nama_expedisi`, `m`.`city` as `kota_member` FROM `transaction_purchase` `tp` JOIN `member` `m` ON `m`.`id` = `tp`.`member_id` JOIN `sales` `s` ON `s`.`id` = `tp`.`sales_id` JOIN `expedisi` `x` ON `x`.`id` = `tp`.`expedisi` ORDER BY `tp`.`dateorder` DESC
 ...
 //nama tabel dari database
	...
	var $column_order = array(null, 'nonota','nama_member','nama_sales','nama_expedisi','kota_member','note'); //field yang ada di table user

	var $column_search = array('nonota','nama_member','nama_sales','nama_expedisi','kota_member','note'); //field yang diizin untuk pencarian 
