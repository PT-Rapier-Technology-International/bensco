...
class Model_master extends CI_Model {
	...
	public function getAllMembersByAbbr($abbreviation){
		$this->db->select('*');
		$this->db->like('uniq_code',$abbreviation);
		$this->db->order_by('id','desc');
		return $this->db->get('member');
	}
	...
	public function getProvByCountry($country_id){
		$this->db->select('*');
	...
	public function getViaExpedisi(){
		$this->db->select('*');
		return $this->db->get('expedisi_via');
	}
	...
	public function getViaExpedisiById($id){
		$this->db->select('*');
		$this->db->where('id',$id);
		return $this->db->get('expedisi_via');
	}
	...
	public function getStokPerusahaanGudangByProduk1($product_id,$perusahaan_gudang_id){
		$this->db->select('*');
	...
	public function getStokPerusahaanGudangByProduk($product_id,$perusahaan_gudang_id){
		$this->db->select('*, sum(stok) as jmlStok');
		$this->db->where('product_id',$product_id);
		$this->db->where('perusahaan_gudang_id',$perusahaan_gudang_id);
		return $this->db->get('product_perusahaan_gudang');
	}
	...
	public function getStokPerusahaanGudangByProduks($product_id,$perusahaan_gudang_id){
		$this->db->select('*, sum(stok) as stokk');
		$this->db->where('product_id',$product_id);
		$this->db->where('perusahaan_gudang_id',$perusahaan_gudang_id);
		return $this->db->get('product_perusahaan_gudang');
	}
	...
	public function getCekStokGudangbyProductPerusahaanLiner($id_produk,$id_perusahaan,$stok){
		$this->db->select('ppg.*,g.name as nama_gudang,g.id as id_gudang, ppg.stok as stok_gudang, sum(ppg.stok) as stok_gudang');
		$this->db->join('perusahaan_gudang pg','ppg.perusahaan_gudang_id = pg.id');
		$this->db->join('gudang g','pg.gudang_id = g.id');
		$this->db->where('ppg.product_id',$id_produk);
	...
	public function getStokbyGudangbyProductPerusahaan($id_produk,$id_perusahaan,$id_gudang){
	...
	public function getGudangIn($id){
		$this->db->select('*');
		$array = explode(',', $id);
