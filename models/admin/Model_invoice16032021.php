...
class Model_invoice extends CI_Model {
	...
	public function getInvoiceDetailByInvoiceId($id){
		...
		$this->db->select('id.*, pc.product_name newname, pc.normal_price netprice, tcp.discount dsc');

		$this->db->join('product pc', 'pc.product_code = id.product_code');

		$this->db->join('transaction_purchase_detail tcp', 'tcp.product_id = pc.id');
...
public function getPaymentTandaTerimaInv($inv){
	...
	public function getTandaTerimaByMembers(){
		...
		// if (isset($_SESSION['rick_auto']['filter_perusahaan_tt']) && $_SESSION['rick_auto']['filter_perusahaan_tt']){
		//
		// 	$this->db->limit(30);
		// }
	...
	public function getTandaTerimaByNoTandaTerima($tandaterima){
		...
		//$this->db->join('invoice_detail idl','idl.id = itt.invoice_id');
	...
	public function getReportKeluar(){
		...
		// return $this->db->query("SELECT id,product_id,transaction_no,produk_beli_id,purchase_detail_id,invoice_id,perusahaan_id,
		// gudang_id,qty_product,stock_input,note,keterangan,create_date FROM report_stok_bm_bl UNION ALL SELECT '' as id , product_id, 
		// kode_rpo as nonota, '' as produk_beli_id, pd.id as purchase_detail_id, '' as invoice_id, perusahaan_id, gudang_id, qty, 
		// '' as stock_input, 'Order Barang Keluar Sementara' as note, 
		// ''as keterangan, p.update_date FROM transaction_purchase p JOIN transaction_purchase_detail pd ON p.id=pd.transaction_purchase_id 
		// WHERE p.status = 0 ORDER BY create_date DESC");
		...
		return $this->db->get('metukabeh rsbmbl');
